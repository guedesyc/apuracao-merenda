...
async function staticApi(path, options = {}) {
  ...
  if (path === "/api/export-maximo") {
    if (!isHmlMode()) throw new Error("A exportacao Excel real precisa da versao com servidor local.");
    const months = [...new Set((db.entries || []).map(entry => String(entry.date || "").slice(0, 7)).filter(Boolean))].sort();
    const rows = [];
    for (const month of months) {
      const days = Number(db.settings?.workingDaysByMonth?.[month] || 22);
      for (const school of db.schools || []) {
        const entries = (db.entries || []).filter(entry => entry.schoolId === school.id && String(entry.date || "").startsWith(month) && isCompleteEntry(entry) && entry.status !== "not_served");
        const values = [month, school.shortName, school.route, days];
        for (const card of db.cards || []) {
          values.push(entries.reduce((maximum, entry) => Math.max(maximum, quantityNumber(entry.quantities?.[card.id])), 0));
        }
        rows.push(values.join(";"));
      }
    }
    const headers = ["mes", "escola", "rota", "dias_uteis", ...(db.cards || []).map(card => `${card.label}_maximo_diario`)];
    const csv = [headers.join(";"), ...rows].join("\n");
    return { ok: true, filename: `hml-maximos-${month}.csv`, contentType: "text/csv; charset=utf-8", base64: btoa(csv) };
  }

  if (path === "/api/export-sem-atendimento") {
    if (!isHmlMode()) throw new Error("A exportacao Excel real precisa da versao com servidor local.");
    ...
    const csv = ["mes;data;escola;rota;nutricionista;motivo", ...rows].join("\n");
    return { ok: true, filename: "hml-sem-atendimento-todos-os-meses.csv", contentType: "text/csv; charset=utf-8", base64: btoa(csv) };
  }
...
function shell(content) {
  ...
  const items = isAdmin
    ? [["dashboard", "Painel"], ["apuracoes", "Apurações"], ["config", "Configurações"], ["exportar", "Exportar"], ["exportar-maximo", "Exportação - Máximo"], ["exportar-sem-atendimento", "Exportação - Sem Atendimento"]]
...
function renderMyMonth() {
  ...
  document.querySelectorAll("[data-month-card-toggle]").forEach(button => {
    button.addEventListener("click", event => {
      const key = event.currentTarget.dataset.monthCardToggle;
      if (state.expandedMonthCards.has(key)) state.expandedMonthCards.delete(key);
      else state.expandedMonthCards.add(key);
      render();
    });
  });
}
            ...
            <div class="school-total-item">
              ...
              <div class="school-card-totals">
                ${cardTotals.length
                    ...
                    <button class="school-card-total-line" type="button" data-month-card-toggle="${school.id}:${card.id}" aria-expanded="${state.expandedMonthCards.has(`${school.id}:${card.id}`)}">
                      <span>${card.label}</span>
                      <strong>${quantity.toLocaleString("pt-BR")}</strong>
                    </button>
                    ${state.expandedMonthCards.has(`${school.id}:${card.id}`) ? monthCardDailyDetails(school.id, card.id) : ""}
                  `).join("")
...
function monthCardDailyDetails(schoolId, cardId) {
  const entries = completeEntriesFor({ month: state.selectedMonth, userId: state.user.id, schoolId })
    .filter(entry => entry.status !== "not_served")
    .map(entry => ({ date: entry.date, quantity: quantityNumber(entry.quantities?.[cardId]) }))
    .filter(item => item.quantity > 0)
    .sort((left, right) => left.date.localeCompare(right.date));
  return `<div class="school-card-daily-list">
    ${entries.length
      ? entries.map(item => `<div><span>${formatDateBR(item.date)}</span><strong>${item.quantity.toLocaleString("pt-BR")}</strong></div>`).join("")
      : `<span class="muted">Nenhuma quantidade registrada nesse card.</span>`}
  </div>`;
}
...
function apuracaoStatus(entry) {
  if (entry.status === "not_served") return { label: "Sem atendimento", className: "warn" };
  if (isCompleteEntry(entry)) return { label: "Registrada", className: "done" };
  return { label: "Pendente", className: "warn" };
}

function adminApuracaoEntry(entry) {
  const school = state.db.schools.find(item => item.id === entry.schoolId);
  const user = state.db.users.find(item => item.id === entry.nutritionistId);
  const status = apuracaoStatus(entry);
  const expanded = state.expandedAdminEntries.has(entry.id);
  const cards = filledCardLines(entry);
  const total = quantitiesTotal(entry.quantities || {});
  return `
    <article class="admin-apuracao-entry ${expanded ? "" : "collapsed"}">
      <button class="admin-apuracao-toggle" type="button" data-admin-apuracao-toggle="${entry.id}" aria-expanded="${expanded}">
        <div class="admin-apuracao-main">
          <strong>${formatDateBR(entry.date)}</strong>
          <div><strong>${school?.shortName || entry.schoolId}</strong><span class="muted">${school?.route || "Sem região"}</span></div>
          <span class="muted">${user?.name || entry.nutritionistName || "Nutricionista não identificada"}</span>
        </div>
        <div class="admin-apuracao-summary">
          <span class="badge ${status.className}">${status.label}</span>
          ${entry.status === "served" && isCompleteEntry(entry) ? `<strong>${money(total)}</strong>` : ""}
          <span class="chevron">${expanded ? "-" : "+"}</span>
        </div>
      </button>
      ${expanded ? `
        <div class="admin-apuracao-detail">
          ${entry.status === "not_served" ? `<strong>Motivo: ${entry.reason || "Não informado"}</strong>` : cards.length ? `
            <div class="card-lines">
              ${cards.map(item => `<div class="card-line"><span>${item.label}</span><strong>${item.qty}</strong><span>${money(quantityNumber(item.qty) * item.price)}</span></div>`).join("")}
            </div>
            <strong>Total da refeição: ${money(total)}</strong>
          ` : `<span class="muted">Nenhum card preenchido.</span>`}
          ${entry.notes ? `<span>Observação: ${entry.notes}</span>` : ""}
        </div>
      ` : ""}
    </article>
  `;
...
function renderApuracoes() {
  ...
  shell(`
    ...
    <section class="panel apuracoes-filters">
      <div class="filters">
        <div class="field" style="min-width: 220px"><label>Nutricionista</label><select id="apuracoes-nutritionist"><option value="todos">Todas</option>${nutritionists().map(user => `<option value="${user.id}" ${state.apuracoesNutritionistFilter === user.id ? "selected" : ""}>${user.name}</option>`).join("")}</select></div>
        <div class="field" style="min-width: 180px"><label>Data</label><input id="apuracoes-date" type="date" value="${state.apuracoesDateFilter}" /></div>
  ...
  document.querySelectorAll("[data-admin-apuracao-toggle]").forEach(button => {
    button.addEventListener("click", event => {
      ...
      if (state.expandedAdminEntries.has(entryId)) state.expandedAdminEntries.delete(entryId);
      else state.expandedAdminEntries.add(entryId);
      renderApuracoes();
    });
  });
}
...
function renderMaximumExport() {
  const history = (state.db.exports || []).filter(item => item.filename.includes("maximos"));
  shell(`
    <div class="topbar">
      <div class="page-title"><h1>Exportação - Máximo</h1><p>Exporte o maior valor diário de cada card por escola e mês.</p></div>
    </div>
    <section class="panel">
      <div class="toolbar">
        <button class="primary" id="export-maximum">Gerar Excel de máximos de todos os meses</button>
      </div>
      <p class="status-line">O máximo considera a maior quantidade diária registrada em cada card, por escola e mês.</p>
      <p class="status-line">O arquivo inclui todos os meses disponíveis e pode ser filtrado pela coluna Mês no Excel.</p>
      <p class="status-line">${state.message}</p>
    </section>
    <section class="panel">
      <h2>Exportações de máximos recentes</h2>
      <div class="table-wrap">
        <table>
          <thead><tr><th>Mês</th><th>Arquivo</th><th>Criado em</th></tr></thead>
      <tbody>${history.slice().reverse().map(item => `<tr><td>${item.month}</td><td><a href="/exports/${item.filename}">${item.filename}</a></td><td>${item.createdAt}</td></tr>`).join("") || `<tr><td colspan="3">Nenhuma exportação de máximos gerada.</td></tr>`}</tbody>
        </table>
      </div>
    </section>
  `);
  $("#export-maximum").addEventListener("click", async () => {
    state.message = "Gerando máximos de todos os meses...";
    renderMaximumExport();
    try {
      const result = await api("/api/export-maximo", { method: "POST", body: {} });
      if (result.base64) downloadBase64File(result);
      state.db = await api("/api/data");
      state.message = `Planilha de máximos gerada: ${result.filename}`;
      renderMaximumExport();
    } catch (error) {
      state.message = error.message;
      renderMaximumExport();
...
function renderNotServedExport() {
  const history = (state.db.exports || []).filter(item => item.filename.includes("sem-atendimento"));
  shell(`
    <div class="topbar">
      <div class="page-title"><h1>Exportação - Sem Atendimento</h1><p>Exporte todas as escolas e datas sem atendimento registrado.</p></div>
    </div>
    <section class="panel">
      <div class="toolbar">
        <button class="primary" id="export-not-served">Gerar Excel de sem atendimento</button>
      </div>
      <p class="status-line">O arquivo reúne todos os meses disponíveis. Use o filtro da coluna Mês para consultar uma competência.</p>
      <p class="status-line">${state.message}</p>
    </section>
    <section class="panel">
      <h2>Exportações de sem atendimento recentes</h2>
      <div class="table-wrap">
        <table>
          <thead><tr><th>Arquivo</th><th>Criado em</th></tr></thead>
          <tbody>${history.slice().reverse().map(item => `<tr><td><a href="/exports/${item.filename}">${item.filename}</a></td><td>${item.createdAt}</td></tr>`).join("") || `<tr><td colspan="2">Nenhuma exportação gerada.</td></tr>`}</tbody>
        </table>
      </div>
    </section>
  `);
  $("#export-not-served").addEventListener("click", async () => {
    state.message = "Gerando relatório de sem atendimento de todos os meses...";
    renderNotServedExport();
    try {
      const result = await api("/api/export-sem-atendimento", { method: "POST", body: {} });
      if (result.base64) downloadBase64File(result);
      state.db = await api("/api/data");
      state.message = `Relatório gerado: ${result.filename}`;
      renderNotServedExport();
    } catch (error) {
      state.message = error.message;
      renderNotServedExport();
...
function render() {
  ...
  if (state.view === "apuracoes") return renderApuracoes();
  if (state.view === "config") return renderConfig();
  if (state.view === "exportar") return renderExport();
  if (state.view === "exportar-maximo") return renderMaximumExport();
  if (state.view === "exportar-sem-atendimento") return renderNotServedExport();
}
