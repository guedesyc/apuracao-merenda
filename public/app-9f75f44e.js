...
async function staticApi(path, options = {}) {
  ...
  if (path === "/api/export-media") {
    ...
    const rows = (db.schools || []).map(school => {
      ...
      const values = [school.shortName, school.route, days];
      for (const card of db.cards || []) {
        const total = entries.reduce((sum, entry) => sum + quantityNumber(entry.quantities?.[card.id]), 0);
        values.push((total / days).toFixed(2));
      }
      return values.join(";");
    });
    const headers = ["escola", "rota", "dias_uteis", ...(db.cards || []).map(card => `${card.label}_media_diaria`)];
    const csv = [headers.join(";"), ...rows].join("\n");
    return { ok: true, filename: `hml-medias-${month}.csv`, contentType: "text/csv; charset=utf-8", base64: btoa(csv) };
  }
...
function shell(content) {
  ...
  const items = isAdmin
    ? [["dashboard", "Painel"], ["config", "Configurações"], ["exportar", "Exportar"], ["exportar-media", "Exportação - Média"]]
...
function renderAverageExport() {
  const history = (state.db.exports || []).filter(item => item.filename.includes("medias"));
  shell(`
    <div class="topbar">
      <div class="page-title"><h1>Exportação - Média</h1><p>Exporte a média diária de cada card por escola.</p></div>
    </div>
    <section class="panel">
      <div class="toolbar">
        <div class="field" style="min-width: 180px"><label>Mês</label><input id="month" type="month" value="${state.selectedMonth}" /></div>
        <button class="primary" id="export-average">Gerar Excel de médias</button>
      </div>
      <p class="status-line">Média calculada pela quantidade registrada em cada card dividida pelos dias úteis da competência.</p>
      <p class="status-line">${state.message}</p>
    </section>
    <section class="panel">
      <h2>Exportações de médias recentes</h2>
      <div class="table-wrap">
        <table>
          <thead><tr><th>Mês</th><th>Arquivo</th><th>Criado em</th></tr></thead>
          <tbody>${history.slice().reverse().map(item => `<tr><td>${item.month}</td><td><a href="/exports/${item.filename}">${item.filename}</a></td><td>${item.createdAt}</td></tr>`).join("") || `<tr><td colspan="3">Nenhuma exportação de médias gerada.</td></tr>`}</tbody>
        </table>
      </div>
    </section>
  `);
  ...
  $("#export-average").addEventListener("click", async () => {
    const month = $("#month").value;
    state.selectedMonth = month;
    state.message = `Gerando médias de ${month}...`;
    renderAverageExport();
    try {
      const result = await api("/api/export-media", { method: "POST", body: { month } });
      if (result.base64) downloadBase64File(result);
      state.db = await api("/api/data");
      state.selectedMonth = month;
      state.message = `Planilha de médias gerada: ${result.filename}`;
      renderAverageExport();
    } catch (error) {
      state.message = error.message;
      renderAverageExport();
...
function render() {
  ...
  if (state.view === "exportar-media") return renderAverageExport();
}
